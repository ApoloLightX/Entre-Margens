extends SceneTree
var game
var output_dir=ProjectSettings.globalize_path('res://../qa/camera_200')
...
func run():
	...
	await settle();game.set_physics_process(false);game.preferences.reduced_motion=true;game.preferences.zoom=1.0;game.camera.zoom=Vector2.ONE
	...
	await shot('poco_f7',Vector2i(2772,1280))
